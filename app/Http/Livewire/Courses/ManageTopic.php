...
class ManageTopic extends Component
{
    ...
    public function save()
    {
        ...
        return redirect()->route('dashboard.courses.topics.edit', [$this->edit_topic->id])->with(['success' => 'Topic Updated.']);
    }

    public function getParents()
    {
        if ($this->edit_topic && isset($this->edit_topic->id)) {
            return CourseTopic::where('id', '!=', $this->edit_topic->id)->select('id', 'name')->get();
        }

        return CourseTopic::select('id', 'name')->get();
    }

    public function render()
    {
        return view(
            'livewire.courses.manage-topic',
            [
