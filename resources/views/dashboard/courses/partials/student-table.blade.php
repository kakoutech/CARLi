    ...
    @if ($enrolled && $enrolled->count())
        ...
        <div class="divide-y divide-gray-200">
            ...
            @foreach ($enrolled as $enroll)
                @php $user = $enroll->learner; @endphp
                    ...
                    <div class="items-center md:flex">
                        ...
                        <div class="flex flex-shrink-0 flex-grow-0 justify-end px-4 md:w-1/12">
                            ...
                            <a class="ml-2 cursor-pointer items-center rounded-full border border-gray-300 bg-white px-2.5 py-0.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50"
                                onclick="if(confirm('Are you sure you want to remove this learner?')) document.getElementById('remove_user_{{ $user->id }}').submit()">
                                Remove </a>

                            <form
                                action="{{ route('dashboard.courses.view.enrolled.remove', [$course->id, $enroll->id]) }}"
                                class="inline" id="remove_user_{{ $user->id }}" method="POST">
                                @csrf
                            </form>
