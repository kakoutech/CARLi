...
@section('content')
    ...
    <div class="items-center justify-between rounded-t-lg bg-brand-500 px-3 py-3 md:flex">
        ...
        <div class="flex flex-col justify-center pl-3 text-white">
            <div class="mb-5 text-xl font-bold leading-none md:mb-0">Articles</div>
        ...
        <div>
            ...
            <div class="gap-3 md:flex">
                ...
                <a class="mb-3 flex items-center rounded-md bg-white py-2 px-5 leading-5 text-gray-800 md:mb-0"
                    href="{{ route('dashboard.strategy-tools.articles.add') }}">
                ...
                <form action="{{ route('dashboard.strategy-tools.articles.mass-delete') }}" method="POST">
                    @csrf
                    @method('DELETE')
                ...
                <form class="relative text-white focus-within:text-gray-600" method="GET">
                    ...
                    <input
                        ...
                        name="search" placeholder="Search Articles" type="search"
        ...
        <div class="flow-root">
            ...
            <div class="hidden items-center bg-gray-200 md:flex">
                ...
                <div class="w-6/12 flex-grow px-4 py-2">
                    ...
                    <span class="block text-sm font-bold uppercase">Article</span>
                        ...
                        <div class="py-4">
                            ...
                            <div class="items-center md:flex">
                                ...
                                <div class="w-12 flex-shrink-0 flex-grow-0 px-4">
                                    <input class="select-me" name="select-me[]" type="checkbox" value="{{ $article->id }}">
                                </div>

                                <div class="flex-grow px-4 md:w-6/12">

                                    <a class="truncate text-lg font-bold text-gray-900"
                                        href="{{ route('dashboard.strategy-tools.articles.view', [$article->id]) }}">{{ $article->getTitle() }}</a>

                                    <p class="truncate text-sm text-gray-500">{{ $article->getPermalink() }}</p>
                                ...
                                <div class="flex-shrink-0 flex-grow-0 px-4 md:w-2/12">
                                    ...
                                    <p class="truncate text-lg font-bold text-gray-900">
                                        <span class="md:hidden">Resource Count:</span>

                                        {{ $article->getResourceCount() }}
                                ...
                                <div class="flex flex-shrink-0 flex-grow-0 justify-end px-4 md:w-1/12">
                                    ...
                                    <a class="cursor-pointer items-center rounded-full border border-gray-300 bg-white px-2.5 py-0.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50"
                                        href="{{ route('dashboard.strategy-tools.articles.view', [$article->id]) }}"> View
                                    </a>
                                    ...
                                    <a class="ml-2 cursor-pointer items-center rounded-full border border-gray-300 bg-white px-2.5 py-0.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50"
                                        href="{{ route('dashboard.strategy-tools.articles.edit', [$article->id]) }}"> Edit
                                    </a>

                                    <a class="ml-2 cursor-pointer items-center rounded-full border border-gray-300 bg-white px-2.5 py-0.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50"
                                        href="#"
                                        onclick="if(confirm('Are you sure you want to delete this page?')) document.getElementById('delete_article_{{ $article->id }}').submit()">
                                        Delete </a>

                                    <form action="{{ route('dashboard.strategy-tools.articles.delete', [$article->id]) }}"
                                        class="inline" id="delete_article_{{ $article->id }}" method="POST">
                                        @csrf
                                        @method('delete')
                                    </form>
            ...
            @else
                <div class="m-5">
                    ...
                    <div class="relative block w-full rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
                        ...
                        <span class="font-mediu mt-2 block text-lg font-bold text-gray-400"> No articles found matching
                            your
                            criteria. </span>
